...
def collect_unique_assets() -> Dict[str, Dict[str, Any]]:
    ...
    manifest_path = os.path.join(ASSETS_DIR, "asset_manifest_v2_clean.csv")
    if os.path.exists(manifest_path):
        with open(manifest_path, encoding="utf-8-sig") as f:
            ...
            for row in reader:
                ...
                if aid in assets:
                    assets[aid]["primary_scene_tag"] = row.get("primary_scene_tag", "")
                    assets[aid]["file_name"] = row.get("file_name", "")
                    assets[aid]["duration_sec"] = float(row.get("duration_sec", 0))
                    assets[aid]["description"] = row.get("description", "")
                    assets[aid]["s3_url"] = row.get("s3_url", "")
...
def is_video_watermark(text: str) -> bool:
    ...
    watermark_patterns = ["LAND", "DOL", "DO!", "ILAND", "ND"]
    return text_upper in watermark_patterns
...
def classify_text(detections: List[Dict[str, Any]], asset_id: str, scene_tag: str) -> Dict[str, Any]:
    ...
    # Determine text type
    if not meaningful_texts or (len(meaningful_texts) <= 2 and all(is_noise_or_artifact(t) or is_video_watermark(t) for t in meaningful_texts)):
        text_type = "none"  # Only noise/watermarks, treat as no meaningful text
...
def determine_text_audit_status(text_type: str, asset_id: str, scene_tag: str, forbidden_texts: List[str], watermark_detected: bool = False) -> Tuple[str, str]:
    ...
    if text_type == "none":
        return "passed", "无检测到有意义的文字（仅噪声/压缩伪影/视频水印）"
    ...
    if text_type == "video_watermark":
        # Video editing watermarks are not user-visible marketing text
        # but they indicate the source may not be professional
        return "passed", "检测到视频编辑软件水印，非营销文字，不影响使用"
...
def check_product_consistency(asset_id: str, scene_tag: str, ocr_texts: List[str]) -> Tuple[str, str]:
    ...
    if scene_tag in PRODUCT_RELATED_TAGS:
        creade_indicators = ["creade", "科瑞德"]
        has_creade = any(ind in combined for ind in creade_indicators)
        if has_creade:
            return "passed", "检测到Creade品牌标识"
        return "passed", "产品相关素材，无其他品牌冲突，来自官方资产库"
    return "passed", "非产品相关标签，产品一致性不适用"
...
def main():
    ...
    logger.info("Collecting unique assets from fix2 runs...")
    assets = collect_unique_assets()
    logger.info(f"Found {len(assets)} unique assets")
    ...
    summary = {
        "audit_version": "1.0", "audit_time": datetime.now().isoformat(),
    ...
    with open(os.path.join(AUDIT_DIR, "material_audit_detail.json"), "w", encoding="utf-8") as f:
        json.dump(audit_results, f, ensure_ascii=False, indent=2)
    with open(os.path.join(AUDIT_DIR, "material_audit_summary.json"), "w", encoding="utf-8") as f:
    ...
    with open(os.path.join(AUDIT_DIR, "native_text_whitelist.json"), "w", encoding="utf-8") as f:
    ...
    with open(os.path.join(AUDIT_DIR, "rejected_assets.json"), "w", encoding="utf-8") as f:
    ...
    with open(os.path.join(AUDIT_DIR, "safe_assets.json"), "w", encoding="utf-8") as f:
