...
def _atomic_json_write(path: str, data: Any) -> None:
    """Write UTF-8 JSON atomically and verify it before replacing the target."""
...
def quality_check_node(
    ...
    if abs(body_sync_diff) > 0.2:
        fail_reasons.append(f"body_sync_diff={body_sync_diff}s>0.2s: 音视频不同步")
    ...
    quality_report = {
        ...
        # 字幕
        "subtitle_render_source": subtitle_render_source,
    ...
    _atomic_json_write(qr_path, quality_report)
