...
- **HTTP 服务**: FastAPI + uvicorn（端口 5000）
- **关键依赖**: coze-coding-dev-sdk, coze-coding-utils, cozeloop, opencv-python, easyocr, boto3, SQLAlchemy
...
## 运行与部署
- **本地运行工作流**: `bash scripts/local_run.sh -m flow`
- **本地运行单节点**: `bash scripts/local_run.sh -m node -n <node_name>`
- **启动 HTTP 服务**: `bash scripts/http_run.sh -p 5000`
- **部署构建**: `bash scripts/setup.sh`
- **部署运行**: `bash scripts/http_run.sh -p 5000`

## 用户偏好与长期约束
- 字幕只来自 subtitles.srt，白色字体(38px)、黑色描边(3px)、底部位置(y=0.82)
- 禁止处理素材画面（不裁切、不缩放、不遮挡、不模糊）
- 素材匹配三级策略：exact→synonym→fallback
- BGM 选择基于 MD5(script_id) 稳定选择，非随机